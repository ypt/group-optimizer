...
var groupFitness = function(groupMembers){
  ...
  // birth year -----------
  // try to make similar (minimize standard deviation)
  var birthYears = _.map(groupMembers, function(memberId){
    return data.getMemberData(memberId).birthYear;
  });

  var birthYearStdDev = ss.standard_deviation(birthYears);
  var birthYearFitness = Math.log(10 - birthYearStdDev) / Math.log(10); // this should max out at 1


  // compute final fitness score -----------
  // apply weights to each facet
  // make sure the max possible is 1
  newFitness = 0.2*genderFitness + 0.3*dayFitness + 0.3*locationFitness + 0.2*birthYearFitness;
